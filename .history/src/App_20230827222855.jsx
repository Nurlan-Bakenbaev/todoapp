...
function App() {
  ...
  const createTodos = async (e) => {
    ...
    await addDoc(collection(db,'todos'),text.input,completed
    com)
  };
  ...
  const style = {
    bg: `h-screen  s-screen p-4 bg-gradient-to-r from-[#ff45e6] to-[#ff7445]`,
    container: `bg-[#e0e0e0] max-w-[600px] w-full m-auto rounded-md shadow-xl p-5 mt-5`,
    heading: `text-3xl font-bold text-center text-gray-800 p-2`,
    form: `flex justify-between items-center`,
    input: `border p-2 w-full text-xl outline-none`,
    button: `p-3 mt-2  text-slate-900`,
    count: "text-center p-3",
    ul: "flex flex-col gap-2 ",
    alert:
      "w-[35%] p-3 text-[red] text-xl border border-solid border-red-500 flex items-center ",
  };
  return (
    <div className={style.bg}>
      <div className={style.container}>
        ...
        {isAlert ? <p className={style.alert}>Enter a valid todo</p> : ""}

        <p className={style.count}>
          You have -
          {todos && todos.length > 1 ? todos.length + " todos " : " todo"}
        </p>
      </div>
    </div>
  );
